...
const navItems = [
  ...
  { href: '/familia', label: 'Mi familia', icon: 'family' },
...
export default async function DashboardLayout({
  ...
  const [{ data: userRow }, { data: profilesRow }] = await Promise.all([
    supabase
      .from('users')
      .select('plan, active_profile_id')
      .eq('id', user.id)
      .single(),
    supabase
      .from('profiles')
  ...
  const profiles = profilesRow ?? []
  const activeProfileId =
    profiles.find((p) => p.id === userRow?.active_profile_id)?.id ??
    profiles.find((p) => p.is_owner)?.id ??
    null
